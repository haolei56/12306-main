 ...
 *
 */
//幂等验证是一种设计原则和机制，用于确保在系统中执行相同操作的多次请求只会产生一次效果，即不会引起重复操作或不一致的结果。
// 在分布式系统、网络通信、数据库操作等场景下，由于网络故障、重试机制或其他原因，可能会导致同一请求被多次执行，从而引发数据重复、资源浪费和逻辑错误。为了避免这种情况，需要使用幂等验证来保证操作的唯一性和一致性。
...
public @interface Idempotent {
     ...
     */
    //这是一个字符串属性，用于指定幂等性验证的关键字。通常用于唯一标识一个操作的幂等性。
    String key() default "";
