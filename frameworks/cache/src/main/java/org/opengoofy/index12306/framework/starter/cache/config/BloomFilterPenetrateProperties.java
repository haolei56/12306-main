 ...
 *
 */
//这段代码定义了一个配置属性类 BloomFilterPenetrateProperties，用于配置布隆过滤器的相关参数。
@Data
@ConfigurationProperties(prefix = BloomFilterPenetrateProperties.PREFIX)//这是 Spring Boot 注解，用于将配置文件中的属性映射到这个类的字段上。prefix 参数指定了属性的前缀，表示只映射带有特定前缀的属性到这个类的字段中。
